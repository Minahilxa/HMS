...
const PatientManagement: React.FC = () => {
  ...
  const [viewTab, setViewTab] = useState<"ehr" | "prescriptions" | "discharge">(
    "ehr",
  );
  ...
  const handleSubmitEHR = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setPatients(updated);
    setSelectedPatient(
      updated.find((p) => p.id === selectedPatient.id) || null,
    );
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Registry, EHR Monitoring, and clinical documentation.
          </p>
        </div>
        {!isNurse && (
          <button
          ...
          {selectedPatient ? (
            <>
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-sm font-medium">
                        ID: #{selectedPatient.id} • Admitted:{" "}
                ...
                <div className="flex border-b border-slate-100 mb-6">
                  {(["ehr", "prescriptions", "discharge"] as const).map(
                    (tab) => (
                      <button
                        key={tab}
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                          ...
                          className="text-sky-600 text-xs font-bold flex items-center hover:underline"
                        >
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                          ...
                          className="text-sky-600 text-xs font-bold flex items-center hover:underline"
                        >
                          <Icons.Prescription className="w-4 h-4 mr-1" /> New
                          Prescription
                        </button>
                      </div>
                      {selectedPatient.prescriptions.length > 0 ? (
                        <div className="space-y-3">
                          {selectedPatient.prescriptions.map((pres) => (
                            <div
                              key={pres.id}
                              className="p-4 rounded-2xl border border-amber-100 bg-amber-50/20"
                            >
                              ...
                              <div className="space-y-2">
                                {pres.medications.map((med, i) => (
                                  <div
                                    key={i}
                                    className="flex justify-between text-xs border-b border-slate-100/50 pb-1"
                                  >
                      ...
                      {selectedPatient.status === PatientStatus.DISCHARGED ? (
                        <div className="space-y-4">
                          <div className="p-4 bg-white rounded-xl border border-slate-100">
                            ...
                            <p className="text-sm font-bold text-slate-800">
                              Acute Respiratory Infection (Resolved)
                            </p>
                          </div>
                          <div className="p-4 bg-white rounded-xl border border-slate-100">
                            <p className="text-xs text-slate-400 uppercase font-bold tracking-wider mb-1">
                            ...
                            <p className="text-sm text-slate-600 leading-relaxed">
                              Continue rest for 5 days. Return if fever persists
                              beyond 72 hours. Scheduled follow-up on June 1st.
                            </p>
                          </div>
                        </div>
                      ) : (
            ...
            <div className="bg-white h-[600px] flex flex-col items-center justify-center rounded-3xl border border-slate-200 shadow-sm p-10 text-center">
              ...
              <p className="text-slate-400 text-sm mt-2 max-w-xs">
                Select a patient from the registry to view their EHR,
                prescriptions, and status.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            ...
            <form onSubmit={handleRegister} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none transition-all h-24 resize-none"
              ...
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmitEHR} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    type="text"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"
                    placeholder="e.g. Lifestyle modification, Medication started"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    name="notes"
                    className="w-full h-24 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none h-24 resize-none"
              ...
              <button
                type="submit"
                className="w-full bg-slate-800 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-black transition-all"
              >
                Save EHR Entry
              </button>
            </form>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-amber-50 border-b border-amber-100 flex justify-between items-center">
              ...
              <button
                ...
                className="text-amber-400 hover:text-amber-600 font-bold text-2xl"
              >
                &times;
              </button>
            </div>
            <form onSubmit={handleSubmitPrescription} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    name="notes"
                    className="w-full h-20 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none resize-none"
              ...
              <button
                type="submit"
                className="w-full bg-amber-600 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-amber-700 transition-all"
              >
