...
const EmergencyManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingCase) {
      const caseId = (editingCase as any)._id || editingCase.id;
      await apiService.updateEmergencyCase(caseId, data);
    } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {isDoctor
              ? "Monitor critical admissions and update patient care status."
              : "Real-time monitoring and coordination of critical hospital admissions."}
          </p>
        </div>
        {!isDoctor && (
          <button
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {cases.map((ec) => {
              const caseId = (ec as any)._id || ec.id;
              return (
                <tr
                  key={caseId}
                ...
                >
                  ...
                  <td className="px-6 py-4 text-xs font-bold text-slate-700">
                    {ec.assignedDoctor || "Triage Pending"}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase w-fit ${
                          ec.status === "Resolved"
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleUpdateStatus(caseId, e.target.value as any)
                        }
            ...
            {cases.length === 0 && (
              <tr>
                <td
                  colSpan={isDoctor ? 5 : 6}
                  className="py-20 text-center text-slate-400 italic font-medium"
                >
                  No active trauma cases currently registered.
                </td>
              </tr>
            )}
          </tbody>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 bg-red-50 border-b border-red-100 flex justify-between items-center">
              ...
              <button
                ...
                className="text-red-400 hover:text-red-600 font-bold text-2xl"
              >
                &times;
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div>
                ...
                <input
                  name="patientName"
                  defaultValue={editingCase?.patientName}
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    name="arrivalType"
                    defaultValue={editingCase?.arrivalType || "Ambulance"}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-red-500 outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    name="priority"
                    defaultValue={editingCase?.priority || "Stable"}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-red-500 outline-none"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    name="assignedDoctor"
                    defaultValue={editingCase?.assignedDoctor}
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-red-500 outline-none"
