...
const PharmacyManagement: React.FC = () => {
  ...
  const [editingSupplier, setEditingSupplier] =
    useState<PharmacySupplier | null>(null);
  ...
  const getExpiryAlert = (expiry: string) => {
    ...
    if (diffDays <= 30)
      return { label: "Expiring Soon", color: "text-amber-700 bg-amber-100" };
    return null;
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Track drug inventory, handle retail billing, and manage
        ...
        <div className="flex p-1 bg-slate-100 rounded-2xl">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "inventory" ? "bg-white text-emerald-600 shadow-sm" : "text-slate-500"}`}
          >
            Inventory
          </button>
          <button
            onClick={() => setActiveTab("sales")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "sales" ? "bg-white text-emerald-600 shadow-sm" : "text-slate-500"}`}
          >
            Sales & Billing
          </button>
          <button
            onClick={() => setActiveTab("suppliers")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "suppliers" ? "bg-white text-emerald-600 shadow-sm" : "text-slate-500"}`}
          >
            Suppliers
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center"
            >
              Add New Medicine
            </button>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <p
                          className={`font-black ${item.stock <= item.minStockLevel ? "text-red-600" : "text-slate-800"}`}
                        >
                          {item.stock}
                        </p>
                      ...
                      <td className="px-6 py-4 text-center text-xs font-medium text-slate-500">
                        ...
                        {expiryAlert && (
                          <span
                            className={`block px-2 py-0.5 mt-1 rounded text-[8px] font-black uppercase ${expiryAlert.color}`}
                          >
                            {expiryAlert.label}
                          </span>
                        )}
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all"
            >
              New Billing
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {sales.map((sale) => (
              <div
                key={sale.id}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-emerald-200 transition-all group"
              >
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {sale.items.map((it, i) => (
                      <span
                        key={i}
                        className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded text-[8px] font-bold"
                      >
                ...
                <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${sale.paymentStatus === "Paid" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {sale.paymentStatus}
                  </span>
            ...
            {suppliers.map((sup) => (
              ...
              >
                ...
                <p className="text-xs text-emerald-600 font-bold mb-4">
                  Lead Contact: {sup.contactPerson}
                </p>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-emerald-50 border-b border-emerald-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-emerald-800">
                {editingItem ? "Edit Medicine" : "Inventory Entry"}
              </h3>
              <button
                onClick={() => setShowAddMedModal(false)}
                className="text-emerald-400 hover:text-emerald-600 font-bold text-2xl"
              >
                &times;
              </button>
            </div>
            <form onSubmit={handleItemSubmit} className="p-8 space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-emerald-700 transition-all mt-4 uppercase tracking-widest text-xs"
              >
      ...
      {showAddSaleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-emerald-50 border-b border-emerald-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-emerald-800">
                {editingSale ? "Edit Billing" : "Retail Sales Entry"}
              </h3>
              <button
            ...
            <form onSubmit={handleSaleSubmit} className="p-8 space-y-4">
              <div>
                ...
                <input
                  name="patientName"
                  defaultValue={editingSale?.patientName}
                  required
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    name="paymentStatus"
                    defaultValue={editingSale?.paymentStatus}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                  >
                    <option>Paid</option>
      ...
      {showAddSupplierModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-emerald-50 border-b border-emerald-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-emerald-800">
                {editingSupplier ? "Edit Vendor" : "New Supplier"}
              </h3>
              <button
            ...
            <form onSubmit={handleSupplierSubmit} className="p-8 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none h-20 resize-none"
                />
              </div>
              <button
                type="submit"
