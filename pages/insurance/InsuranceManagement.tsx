...
const InsuranceManagement: React.FC = () => {
  ...
  const handlePanelSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const panelData = {
      ...
      settlementPeriod:
        parseInt(formData.get("settlementPeriod") as string) || 30,
  ...
  const handleDeletePanel = async (id: string) => {
    if (
      confirm(
        "Are you sure you want to delete this insurance panel? This action cannot be undone.",
      )
    ) {
  ...
  const handleClaimUpdate = async (
    ...
    if (status === "Approved" || status === "Settled") {
      ...
      const amt = prompt(
        "Enter Approved/Settled Amount:",
        currentClaim?.claimAmount.toString(),
      );
      if (amt === null) return;
  ...
  const handleRegisterCoverage = async (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Corporate panels management, patient eligibility verification, and
        ...
        <div className="flex p-1 bg-slate-100 rounded-2xl">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "panels" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
          >
            Panel Directory
          </button>
          <button
            onClick={() => setActiveTab("claims")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "claims" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
          >
            Claim Tracking
          </button>
          <button
            onClick={() => setActiveTab("verification")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "verification" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
          >
            Patient Coverage
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center"
            >
              Register New Panel
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {panels.map((panel) => (
              <div
                key={panel.id}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-indigo-200 transition-all group relative"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase px-2 py-1 rounded-lg">
                    {panel.code}
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${panel.status === "Active" ? "bg-green-500" : "bg-red-500"}`}
                    ></span>
                    <button
                      onClick={() => {
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-4">
                  Settlement: {panel.settlementPeriod} Days
                </p>
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`px-2 py-1 rounded-full text-[10px] font-black uppercase w-fit ${
                      ...
                      <select
                        value={claim.status}
                        onChange={(e) =>
                          handleClaimUpdate(claim.id, e.target.value as any)
                        }
                    ...
                    <div className="flex justify-center gap-1">
                      {claim.status === "Pending" && (
                        <>
                          <button
                            onClick={() =>
                              handleClaimUpdate(claim.id, "Approved")
                            }
                            className="bg-sky-50 text-sky-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-sky-100 transition-colors"
                          >
                            Quick Approve
                          </button>
                          <button
                            onClick={() =>
                              handleClaimUpdate(claim.id, "Rejected")
                            }
                            className="bg-red-50 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors"
                          >
                            Reject
                          </button>
                        </>
                      ...
                      {claim.status === "Approved" && (
                        <button
                          onClick={() => handleClaimUpdate(claim.id, "Settled")}
                          className="bg-green-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-green-700 transition-colors"
                        >
                          Mark Settled
                        </button>
                      )}
              ...
              {claims.length === 0 && (
                <tr>
                  ...
                  >
                    No insurance claims registered.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-4 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm h-fit">
            ...
            <form onSubmit={handleVerifyPatient} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-indigo-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-indigo-700 transition-all uppercase tracking-widest text-xs"
              >
            ...
            <div className="mt-8 pt-8 border-t border-slate-50">
              <button
                ...
                className="w-full bg-slate-100 text-slate-600 font-bold py-3 rounded-xl text-xs hover:bg-slate-200 transition-all uppercase tracking-widest"
              >
            ...
            {selectedVerification ? (
              <div className="space-y-6 animate-in fade-in duration-200">
                {selectedVerification.length > 0 ? (
                  selectedVerification.map((cov) => (
                    <div
                      key={cov.id}
                      className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden"
                    >
                      <div className="absolute top-0 right-0 p-4">
                        <span className="bg-green-50 text-green-600 text-[10px] font-black px-3 py-1 rounded-full border border-green-100 uppercase">
                          {cov.status}
                        </span>
                      </div>
                      <div className="flex flex-col md:flex-row justify-between gap-6">
                        <div>
                          ...
                          <p className="text-indigo-600 font-bold mt-1">
                            {panels.find((p) => p.id === cov.panelId)?.name}
                          </p>
                        </div>
                      ...
                      <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          ...
                          <p className="text-lg font-black text-amber-600">
                            ${cov.consumedLimit.toLocaleString()}
                          </p>
                        </div>
                        <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 col-span-2">
                          ...
                          <p className="text-lg font-black text-indigo-600">
                            $
                            {(
                              cov.totalLimit - cov.consumedLimit
                            ).toLocaleString()}
                          </p>
                        </div>
                      </div>
              ...
              <div className="bg-slate-50/50 h-[400px] rounded-3xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="italic text-sm">
                  Use the search panel to verify patient eligibility.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4">
            <div className="p-6 bg-indigo-50 border-b border-indigo-100 flex justify-between items-center">
              ...
              <button
                ...
                className="text-indigo-400 hover:text-indigo-600 font-bold text-2xl"
              >
                &times;
              </button>
            </div>
            <form onSubmit={handlePanelSubmit} className="p-8 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    name="code"
                    defaultValue={editingPanel?.code}
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
              ...
              <div>
                ...
                <input
                  name="contactPerson"
                  defaultValue={editingPanel?.contactPerson}
                  required
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    name="phone"
                    defaultValue={editingPanel?.phone}
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    name="email"
                    type="email"
                    defaultValue={editingPanel?.email}
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    type="number"
                    defaultValue={editingPanel?.settlementPeriod || 30}
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
                <div>
                  ...
                  <select
                    name="status"
                    defaultValue={editingPanel?.status || "Active"}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                  >
