...
const BillingManagement: React.FC = () => {
  ...
  const handleInsuranceAction = async (
    id: string,
    status: Invoice["insuranceStatus"],
  ) => {
  ...
  const filteredInvoices =
    activeTab === "ALL"
      ? invoices
      : invoices.filter((inv) => inv.category === activeTab);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Manage OPD/IPD invoicing, diagnostic billing, and insurance claims.
          </p>
        </div>
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredInvoices.map((inv) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span className="bg-slate-100 px-3 py-1 rounded-full text-[10px] font-black text-slate-600 uppercase tracking-tighter">
                    {inv.category}
                  </span>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                    <span
                      className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase w-fit ${
                        inv.status === "Paid"
                    ...
                    {inv.paymentMethod === "Insurance" && (
                      <span
                        className={`px-2 py-0.5 rounded text-[8px] font-bold border ${
                          inv.insuranceStatus === "Approved"
                            ? "border-green-200 text-green-700 bg-green-50"
                            : inv.insuranceStatus === "Rejected"
                              ? "border-red-200 text-red-700 bg-red-50"
                              : "border-amber-200 text-amber-700 bg-amber-50"
                        }`}
                      >
                        {inv.insuranceProvider || "Corporate"} -{" "}
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-1">
                    ...
                    {inv.paymentMethod === "Insurance" &&
                      inv.insuranceStatus === "Pending" && (
                        <div className="flex gap-1">
                          <button
                            onClick={() =>
                              handleInsuranceAction(inv.id, "Approved")
                            }
                            className="bg-green-50 text-green-600 px-2 py-1.5 rounded-lg text-[10px] font-bold hover:bg-green-100 transition-colors"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() =>
                              handleInsuranceAction(inv.id, "Rejected")
                            }
                            className="bg-red-50 text-red-600 px-2 py-1.5 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors"
                          >
                            Reject
                          </button>
                        </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4">
            ...
            <form onSubmit={handleCreateInvoice} className="p-8 space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex justify-between items-center">
                ...
                <button
                  type="submit"
                  className="bg-slate-900 text-white font-bold px-8 py-3 rounded-2xl shadow-lg hover:bg-black transition-all uppercase tracking-widest text-xs"
                >
                  Finalize & Print
                </button>
              </div>
