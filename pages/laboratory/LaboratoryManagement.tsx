      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Diagnostic track, sample collection, and automated result reporting.
          </p>
        </div>
        <div className="flex p-1 bg-slate-100 rounded-2xl overflow-x-auto whitespace-nowrap">
          <button
            onClick={() => setActiveSubTab("tests")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "tests" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Test Directory
          </button>
          <button
            onClick={() => setActiveSubTab("samples")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "samples" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Sample Tracking
          </button>
          <button
            onClick={() => setActiveSubTab("results")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "results" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Diagnostics
          </button>
          <button
            onClick={() => setActiveSubTab("history")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "history" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Portal Logs
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-end">
            <button
              ...
              className="bg-sky-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all flex items-center"
            >
              <svg
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tests.map((test) => (
              <div
                key={test.id}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-sky-200 transition-all group"
              >
                <div className="flex justify-between items-start mb-4">
                  <span className="bg-sky-50 text-sky-600 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg">
                    {test.category}
                  </span>
                ...
                <div className="mt-6 pt-4 border-t border-slate-50 flex justify-between">
                  <button className="text-[10px] font-bold text-slate-400 hover:text-sky-600 uppercase">
                    Edit Pricing
                  </button>
                  <button className="text-[10px] font-bold text-red-400 hover:text-red-600 uppercase">
                    Archive
                  </button>
                </div>
              </div>
            ...
            <tbody className="divide-y divide-slate-50">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <select
                        value={sample.status}
                        onChange={(e) =>
                          handleManualStatusChange(sample.id, e.target.value)
                        }
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-1">
                        {sample.status === "Pending" && (
                          <button
                            onClick={() =>
                              handleUpdateStatus(sample.id, "Collected")
                            }
                            className="bg-sky-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold"
                          >
                            Collect Sample
                          </button>
                        )}
                        {sample.status === "Collected" && (
                          <button
                            onClick={() =>
                              handleUpdateStatus(sample.id, "In Progress")
                            }
                            className="bg-indigo-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold"
                          >
                            Move to Lab
                          </button>
                        )}
                        {sample.status === "In Progress" && (
                          <button
                            onClick={() =>
                              handleUpdateStatus(sample.id, "Completed")
                            }
                            className="bg-green-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold"
                          >
                            Enter Results
                          </button>
                        )}
              ...
              {samples.filter((s) => s.status !== "Completed").length === 0 && (
                <tr>
                  ...
                  >
                    No samples currently in diagnostic pipeline.
                  </td>
                </tr>
              )}
            </tbody>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-black text-sky-700">
                        {sample.result || "Pending Findings Entry"}
                      </p>
              ...
              {samples.filter((s) => s.status === "Completed").length === 0 && (
                <tr>
                  <td
                    ...
                    className="p-10 text-center text-slate-400 italic font-medium"
                  >
                    No diagnostic results verified yet.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            >
              ...
              <button
                type="submit"
                className="w-full bg-slate-800 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-slate-900 transition-all uppercase tracking-widest text-xs"
              >
                Register Test Entry
              </button>
            </form>
