...
const DepartmentManagement: React.FC = () => {
  ...
  const [editingDept, setEditingDept] = useState<HospitalDepartment | null>(
    null,
  );
  const [editingService, setEditingService] = useState<HospitalService | null>(
    null,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Manage institutional infrastructure, specialized departments, and
            service offerings.
          </p>
        </div>
        <div className="flex gap-3">
          <div className="flex p-1 bg-slate-100 rounded-2xl">
            <button
              ...
              className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "depts" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
              Departments
            </button>
            <button
              onClick={() => setActiveTab("services")}
              className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "services" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
              Services & Facilities
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {departments.map((dept) => {
            ...
            return (
              <div
                key={dept.id}
                className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm relative group hover:border-sky-200 transition-all"
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${dept.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
                      {dept.status}
                    </span>
                  </div>
                ...
                <p className="text-sm text-slate-500 mt-2 mb-6 line-clamp-2">
                  {dept.description}
                </p>
            ...
            <tbody className="divide-y divide-slate-50">
              {services.map((service) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className="bg-slate-100 px-3 py-1 rounded-full text-[10px] font-bold text-slate-600 uppercase tracking-tighter">
                      {service.category}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${service.isAvailable ? "text-green-600 bg-green-50" : "text-red-500 bg-red-50"}`}
                    >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-2">
                      <button
                        onClick={() =>
                          toggleServiceStatus(service.id, service.isAvailable)
                        }
              ...
              {services.length === 0 && (
                <tr>
                  ...
                  >
                    No services registered yet. Click 'Add Service' to begin.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleDeptSubmit} className="p-8 space-y-6">
              ...
              <div>
                ...
                <select
                  name="headDoctorId"
                  defaultValue={editingDept?.headDoctorId}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"
                >
      ...
      {showServiceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-slate-800">
                {editingService ? "Edit" : "New"} Facility Service
              </h3>
              <button
            ...
            <form onSubmit={handleServiceSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    name="category"
                    defaultValue={editingService?.category || "Diagnostic"}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none h-24 resize-none"
                  placeholder="Explain the service scope..."
                />
              </div>
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-sky-700 transition-all uppercase tracking-widest text-xs"
              >
