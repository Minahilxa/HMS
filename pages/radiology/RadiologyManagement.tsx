      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Diagnostic imaging orders, radiologist notes, and report archival.
          </p>
        </div>
        <div className="flex p-1 bg-slate-100 rounded-2xl">
          <button
            onClick={() => setActiveSubTab("pending")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "pending" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Current Studies
          </button>
          <button
            onClick={() => setActiveSubTab("completed")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeSubTab === "completed" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Archived Reports
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-end">
            <button
              ...
              className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg hover:bg-black transition-all flex items-center"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pendingOrders.map((order) => (
              <div
                key={order.id}
                className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:border-sky-300 transition-all group"
              >
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      {order.status === "Requested" && (
                        <button
                          onClick={() =>
                            handleUpdateStatus(order.id, "Scheduled")
                          }
                          className="bg-sky-50 text-sky-600 px-3 py-1.5 rounded-lg text-[10px] font-bold"
                        >
                          Schedule
                        </button>
                      )}
                      {order.status === "Scheduled" && (
                        <button
                          ...
                          className="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold"
                        >
                          Capture Study
                        </button>
                      )}
                      {order.status === "Completed" && (
                        <button
                          onClick={() =>
                            handleUpdateStatus(order.id, "Reported")
                          }
                          className="bg-green-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold"
                        >
                          Write Report
                        </button>
                      )}
            ...
            {pendingOrders.length === 0 && (
              <div className="col-span-full py-16 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-100 rounded-3xl">
                ...
                <p className="text-sm italic">
                  No active imaging studies in queue.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4">
            ...
            <form onSubmit={handleCreateOrder} className="p-8 space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                  Study Description (Body Part/View)
                </label>
                <input
                  name="bodyPart"
                  required
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"
                  placeholder="e.g. Spine Lumbar AP/LAT"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-sky-700 transition-all"
              >
