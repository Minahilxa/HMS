...
const CommunicationManagement: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"announcements" | "sms" | "email">(
    "announcements",
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {isReadOnly
              ? "View institutional broadcasts and patient communication logs."
              : "Manage internal staff broadcasts and monitor patient outreach via SMS/Email."}
          </p>
        </div>
        <div className="flex p-1 bg-slate-100 rounded-2xl overflow-x-auto whitespace-nowrap">
          <button
            onClick={() => setActiveTab("announcements")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "announcements" ? "bg-white text-sky-600 shadow-sm" : "text-slate-50"}`}
          >
            Announcements
          </button>
          <button
            onClick={() => setActiveTab("sms")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "sms" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            SMS Gateway
          </button>
          <button
            onClick={() => setActiveTab("email")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "email" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Email Alerts
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {announcements.map((ann) => (
              <div
                key={ann.id}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-sky-200 transition-all relative group"
              >
                ...
                {smsLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 max-w-md">
                      ...
                      <span className="text-[8px] text-sky-600 font-black uppercase mt-1 inline-block">
                        {log.type}
                      </span>
                    </td>
          ...
          <div className="flex justify-between items-center">
            <div className="flex gap-4">
              ...
              <div className="flex p-0.5 bg-slate-100 rounded-xl">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${emailView === "inbox" ? "bg-white text-sky-600 shadow-sm" : "text-slate-400"}`}
                >
                  Inbox
                </button>
                <button
                  onClick={() => setEmailView("sent")}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${emailView === "sent" ? "bg-white text-sky-600 shadow-sm" : "text-slate-400"}`}
                >
                  Sent
                </button>
              </div>
            ...
            {!isReadOnly && (
              <button
                ...
                className="bg-sky-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-sky-700 transition-all flex items-center"
              >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">
                    {emailView === "inbox" ? "Sender" : "Recipient"}
                  </th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">
                    Subject & Preview
                  </th>
                    ...
                    >
                      <td className="px-6 py-4">
                        <p className="font-bold text-slate-800">
                          {log.patientName ||
                            (emailView === "inbox"
                              ? log.senderEmail.split("@")[0]
                              : log.recipientEmail.split("@")[0])}
                        </p>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span
                          className={`px-2 py-1 rounded-full text-[8px] font-black uppercase ${
                            log.status === "Opened" || log.status === "Received"
                              ? "bg-green-50 text-green-600"
                              : log.status === "Sent"
                                ? "bg-blue-50 text-blue-600"
                                : "bg-red-50 text-red-600"
            ...
            <form onSubmit={handleCreateAnnouncement} className="p-8 space-y-6">
              ...
              <div>
                ...
                <textarea
                  name="content"
                  required
                  className="w-full h-32 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none resize-none"
                />
              </div>
              <button
                type="submit"
            ...
            <form onSubmit={handleSendSMS} className="p-8 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Type your patient message here..."
                />
              </div>
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-sky-700 transition-all flex items-center justify-center"
              >
            ...
            <div className="p-6 bg-slate-900 border-b border-slate-800 flex justify-between items-center">
              <div className="text-white">
                <h3 className="text-xl font-bold">
                  Compose Medical Report / Alert
                </h3>
                <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-1">
                  From: abbasminahil1@gmail.com
                </p>
              </div>
              <button
                onClick={() => setShowEmailModal(false)}
                className="text-slate-500 hover:text-white font-bold text-2xl"
              >
                &times;
              </button>
            </div>
            <form onSubmit={handleSendEmail} className="p-8 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"
                  placeholder="e.g. Critical Lab Results Available"
                />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                ...
                <textarea
                  ...
                  placeholder="Type clinical content here..."
                />
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                <div className="flex gap-2">
                  <button
                    type="button"
                    className="p-2 bg-slate-50 text-slate-400 rounded-lg hover:text-sky-600 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="p-2 bg-slate-50 text-slate-400 rounded-lg hover:text-sky-600 transition-colors"
                  >
                    <Icons.Radiology className="w-5 h-5" />
                  </button>
                </div>
                <button
                  type="submit"
                  className="bg-sky-600 text-white px-8 py-3 rounded-2xl font-bold shadow-lg hover:bg-sky-700 transition-all uppercase text-xs tracking-widest"
                >
                  Dispatch Email
                </button>
              </div>
