...
const ReportsManagement: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "revenue" | "performance" | "departments" | "growth" | "custom"
  >("revenue");
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Institutional health metrics, clinical performance, and financial
            auditing.
          </p>
        </div>
        <div className="flex p-1 bg-slate-100 rounded-2xl overflow-x-auto whitespace-nowrap">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "revenue" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Revenue
          </button>
          <button
            onClick={() => setActiveTab("performance")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "performance" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Clinicians
          </button>
          <button
            onClick={() => setActiveTab("departments")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "departments" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Depts
          </button>
          <button
            onClick={() => setActiveTab("growth")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "growth" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Growth
          </button>
          <button
            onClick={() => setActiveTab("custom")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "custom" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}
          >
            Report Mgmt
          </button>
        </div>
      ...
      {activeTab === "performance" && (
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-8">
            Clinician Patient Load Analysis
          </h3>
                ...
                {depts.map((dept) => (
                  <div key={dept.id}>
                    <div className="flex justify-between text-xs font-black uppercase mb-2">
                      <span>{dept.name}</span>
                      <span>
                        {Math.floor(Math.random() * 40 + 60)}% Utilization
                      </span>
                    </div>
      ...
      {activeTab === "growth" && (
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-8">
            Registration vs. Discharge Velocity
          </h3>
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="bg-sky-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reports.map((report) => (
              <div
                key={report.id}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-sky-300 transition-all group"
              >
                <div className="flex justify-between mb-4">
                  <span className="bg-sky-50 text-sky-600 text-[10px] font-black uppercase px-2 py-1 rounded-lg">
                    {report.type}
                  </span>
                  <button
                ...
                <div className="mt-4 pt-4 border-t border-slate-50">
                  ...
                  <button className="w-full bg-slate-100 text-slate-600 font-black text-[10px] py-2 rounded-xl uppercase tracking-widest hover:bg-sky-600 hover:text-white transition-all">
                    Generate Now
                  </button>
                </div>
              </div>
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Annual Surgical Audit"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Department IN (Cardiology, Surgery) AND Status = 'Finalized'"
                />
              </div>
              <button
                type="submit"
