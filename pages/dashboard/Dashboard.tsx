  ...
  const getGreeting = () => {
    switch (role) {
      case UserRole.DOCTOR:
        return "Doctor's Clinical Hub";
      case UserRole.PATIENT:
        return "My Patient Portal";
      case UserRole.ACCOUNTANT:
        return "Financial Intelligence Unit";
      case UserRole.LAB_TECH:
        return "Laboratory Operations Control";
      default:
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Welcome back, {currentUser?.name || "Administrator"}. Here is your
            session summary.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <DashboardCard
          title={
            role === UserRole.PATIENT ? "My Appointments" : "Daily Appointments"
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {revenue && revenue.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-100">
                  ...
                  <p className="text-xs italic font-medium">
                    Synchronizing with Billing records...
                  </p>
                </div>
              )}
            </div>
        ...
        <div
          className={`${role === UserRole.SUPER_ADMIN || role === UserRole.ADMIN || role === UserRole.ACCOUNTANT ? "lg:col-span-1" : "lg:col-span-3"} bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col`}
        >
          <div className="flex justify-between items-center mb-6">
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
            ...
            <span className="bg-green-100 text-green-700 px-4 py-1.5 rounded-full text-xs font-black">
              {displayStats.doctorsOnDuty}
            </span>
          </div>
        </div>
      </div>
