...
const DoctorManagement: React.FC = () => {
  const [activeSubTab, setActiveSubTab] = useState<
    "profiles" | "schedules" | "leaves" | "performance"
  >("profiles");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Administrative oversight for clinical staff.
          </p>
        </div>
          ...
          {doctors.map((doctor) => (
            ...
            >
              <div className="flex items-center mb-6">
                <div className="w-10 h-10 rounded-full bg-sky-100 flex items-center justify-center text-sky-600 font-bold mr-3">
                  {doctor.name[4]}
                </div>
                <h3 className="font-bold text-slate-800">
                  {doctor.name} - OPD Schedule
                </h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                {[
                  ...
                  return (
                    <div
                      key={day}
                      className={`p-4 rounded-2xl border ${daySched ? "border-sky-100 bg-sky-50/30" : "border-slate-50 bg-slate-50/10"}`}
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {performance.map((perf) => {
            ...
            return (
              <div
                key={perf.doctorId}
                className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative"
              >
                ...
                <div className="space-y-4 relative z-10">
                  <div>
                    ...
                    <div className="h-1.5 w-full bg-slate-100 rounded-full">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (perf.patientsSeen / 500) * 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex justify-between items-center border-t border-slate-50">
                    ...
                    <span className="text-xs text-slate-400">
                      {perf.surgeriesPerformed} Surgeries
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-slate-800">
                {editingDoctor ? "Edit" : "Add New"} Doctor Profile
              </h3>
              <button
                onClick={() => {
                  setShowDoctorModal(false);
            ...
            >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                  Public Biography (for Website)
                </label>
                <textarea
                  name="publicBio"
                  defaultValue={editingDoctor?.publicBio}
              ...
              <div className="flex items-center gap-3 py-2">
                <input
                  ...
                  name="displayOnWeb"
                  id="displayOnWeb"
                  defaultChecked={editingDoctor?.displayOnWeb}
                ...
                >
                  Display this profile on the public hospital website
                </label>
              </div>
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-sky-700 transition-all mt-4 uppercase tracking-widest text-xs"
              >
