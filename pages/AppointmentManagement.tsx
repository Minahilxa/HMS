...
const AppointmentManagement: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"online" | "walk-in" | "slots">(
    "online"
  );
  ...
  const handleStatusUpdate = async (
    ...
    // Ensuring we use the correct ID string passed from the click handler
    if (!id || id === "undefined") {
      console.error(
        "Critical: Attempted to update appointment with undefined ID"
      );
      return;
    }
    ...
    if (success) {
      setAppointments((prev) =>
        prev.map((a) => {
          const aptId = (a as any)._id || a.id;
          return aptId === id ? { ...a, status } : a;
        })
      );
    }
  };
  ...
  const handleCreateAppointment = async (
    ...
    if (isPatient) {
      ...
      const doc = doctors.find(
        (d) => (d as any)._id === doctorId || d.id === doctorId
      );
      ...
      doctorId = matchedDoc
        ? (matchedDoc as any)._id || matchedDoc.id
        : "custom-input";
    }

    const initialStatus: Appointment["status"] = isPatient
      ? "Pending Approval"
      : "Scheduled";
  ...
  const handleCreateSlot = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const doctorNameInput = formData.get("doctorName") as string;
    const matchedDoc = doctors.find(
      (d) => d.name.toLowerCase() === doctorNameInput.toLowerCase()
    );
    const doctorId = matchedDoc
      ? (matchedDoc as any)._id || matchedDoc.id
      : "custom-input";
  ...
  const filteredApts = appointments.filter((a) => {
    const matchesSource =
      activeTab === "online"
        ? a.source === AppointmentSource.ONLINE
        : a.source === AppointmentSource.WALK_IN;
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Coordinate patient consultations and doctor availability.
          </p>
        </div>
        <button
      ...
      {activeTab !== "slots" ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredApts.map((apt) => {
                const aptId = (apt as any)._id || apt.id;
                return (
                  <tr
                    key={aptId}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-1">
                        {apt.status === "Pending Approval" && isAdmin && (
                          <button
                            onClick={() =>
                              handleStatusUpdate(aptId, "Scheduled")
                            }
                            className="bg-purple-50 text-purple-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-purple-100"
                          >
                            Approve
                          </button>
                        )}
                        {apt.status === "Scheduled" && !isPatient && (
                          <button
                            onClick={() =>
                              handleStatusUpdate(aptId, "Checked-in")
                            }
                            className="bg-sky-50 text-sky-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-sky-100"
                          >
                            Check-in
                          </button>
                        )}
                        {apt.status === "Checked-in" && !isPatient && (
                          <button
                            onClick={() =>
                              handleStatusUpdate(aptId, "Completed")
                            }
                            className="bg-green-50 text-green-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-green-100"
                          >
                            Complete
                          </button>
                        )}
                        {apt.status !== "Cancelled" &&
                          apt.status !== "Completed" && (
                            <button
                              onClick={() =>
                                handleStatusUpdate(aptId, "Cancelled")
                              }
                              className="bg-red-50 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-red-100"
                            >
                              Cancel
                            </button>
                          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {slots.map((slot) => {
            const slotId = (slot as any)._id || slot.id;
            const doc = doctors.find(
              (d) => (d as any)._id === slot.doctorId || d.id === slot.doctorId
            );
            return (
              <div
                key={slotId}
              ...
              >
                ...
                <p className="text-[10px] text-sky-600 font-bold uppercase tracking-widest">
                  {doc?.name || "Dr. Unknown"}
                </p>
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 flex justify-between items-center">
                  ...
                  <button className="text-sky-600 text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                    Edit
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-slate-800">
                New {activeTab === "slots" ? "General" : activeTab} Appointment
              </h3>
              <button
            ...
            <form onSubmit={handleCreateAppointment} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                    {isPatient ? "Select Doctor" : "Doctor Name"}
                  </label>
                  {isPatient ? (
                    <select
                      name="doctorId"
              ...
              <button
                type="submit"
                className="w-full bg-sky-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all uppercase tracking-widest text-xs"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleCreateSlot} className="p-8 space-y-6">
              ...
              <button
                type="submit"
                className="w-full bg-slate-800 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-black transition-all"
              >
                Initialize Slot
              </button>
            </form>
